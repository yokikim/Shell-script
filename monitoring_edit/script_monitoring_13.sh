...
HIGHLIGHT_HOSTNAME="\033[1;38;5;171;48;5;24m$HOSTNAME\033[22;0m"
...
function FUNC_PRINT_SYSINFO {
	printf "\033[3m$1\033[23m: \033[1;38;5;135m$2 \033[22;39;0m|"
}
...
TITLE_MONITORING "Basic Information"
	...
	FUNC_PRINT_SYSINFO "Hostname" "$HOSTNAME"
	FUNC_PRINT_SYSINFO "KernelVer" "$(uname -r)"
	FUNC_PRINT_SYSINFO "OSVer" "$(awk '{ printf "%.1f" , $6 }' /etc/redhat-release)"
	printf "\n\n"
	## 하이버바이저 여부 확인
...
function MULTIPATH_USEAGE_CHECK {
	...
	if [ $MULTIPATH_SERV == 1 ] && [ $MULTIPATH_PKG == 1 ] && [ $MULTIPATH_CFG == 1 ] ; then
		...
		MULTIPATH_WWID=$(awk '/^[^#]\swwid/ {print $2}' /etc/multipath.conf)
		## 링크가 떨어진 멀티패스 링크 확인
		...
		elif [ 0 == $(multipath -ll | grep "$MULTIPATH_ALIAS" | wc -l) ] ; then
			FUNC_INDENT
